...
pub async fn establish(path: PathBuf, strict: bool) -> surrealdb::Result<Surreal<Db>> {
    match strict {
        true => {
        ...
        false => {
            let conn = Surreal::new::<SpeeDb>((path.join("db"), Config::default())).await?;
            conn.use_ns("manread").use_db("manread").await?;
            Ok(conn)
        }
    }
}
