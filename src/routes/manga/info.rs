...
pub async fn info(Json(req): Json<MangaInfoRequest>, manga: Data<MangaDBService>, tags_s: Data<TagDBService>, user: ReqData<Claim>, kind_s: Data<MangaKindDBService>, user_s: Data<UserDBService>, uri: Data<UriService>) -> ApiResult<Json<MangaInfoResponse>> {
    let manga = manga.get(req.manga_id.as_str()).await?;
    let kind = kind_s.get_kind(&manga.data.kind.thing.id().to_string()).await.ok_or(ApiError::db_error())?;
    let mut tags = vec![];

    for tag in manga.data.tags {
        tags.push(tags_s.get_tag(&tag.thing.id().to_string()).await.ok_or(ApiError::db_error())?.to_public());
    }
    ...
    for artist in manga.data.artists {
        artists.push(user_s.get_username(&artist.thing.id().to_string()).await.ok_or(ApiError::db_error())?);
    }
    let mut authors = vec![];
    for author in manga.data.authors {
        authors.push(user_s.get_username(&author.thing.id().to_string()).await.ok_or(ApiError::db_error())?);
    }
    let mut chapters = vec![];
    for chapter in manga.data.chapters {
        let v: Chapter = chapter.thing.get(&*tags_s.conn).await?.ok_or(ApiError::db_error())?;
        let mut ctags = vec![];
        for tag in v.tags {
            ctags.push(tags_s.get_tag(&tag.thing.id().to_string()).await.ok_or(ApiError::db_error())?.to_public());
        }
    ...
    Ok(Json(MangaInfoResponse {
        ...
        uploader: user_s.get_username(&manga.data.uploader.thing.id().to_string()).await.ok_or(ApiError::db_error())?,
        artists,
        authors,
        cover: pos as u32,
        cover_ext: manga.data.covers.get(pos).unwrap().clone(),
        chapters,
        sources: manga.data.sources.into_iter().map(|url| {
        ...
        relations: manga.data.relations.into_iter().map(|v| (v.thing.id().to_string(), "".to_string())).collect(),
        favorite: false,
        progress: None,
    }))
}