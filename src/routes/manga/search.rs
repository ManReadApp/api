...
async fn search(Json(request): Json<SearchRequest>, manga: Data<MangaDBService>, tags: Data<TagDBService>) -> ApiResult<Json<Vec<SearchResponse>>> {
    Ok(Json(format(manga.search(request).await?, &tags)))
}
